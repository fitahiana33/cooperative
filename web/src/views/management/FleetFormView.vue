...
const title = computed(() => `${editing.value ? 'Modifier' : 'Ajouter'} ${{
  vehicules: 'un vehicule', chauffeurs: 'un chauffeur', marques: 'une marque', modeles: 'un modele',
}[section.value]}`)
...
const selectedVehicleBrand = computed(() => selectedVehicleModel.value?.marque || marques.value.find(item => item.id === selectedVehicleModel.value?.id_marque))
...
async function openExistingDocument(document: VehiculeDocument) {
  try {
  ...
  } catch (errorValue: unknown) { showError(errorValue, 'Impossible d ouvrir le document.') }
}
...
async function loadReferences() {
  ...
  const requiredIndexes = section.value === 'vehicules' ? [1, 2] : section.value === 'chauffeurs' ? [2] : section.value === 'modeles' ? [0] : []
  const failedReference = requiredIndexes.map(index => results[index]).find(result => result.status === 'rejected')
  if (failedReference?.status === 'rejected') throw failedReference.reason
}

async function loadEligibleUsers() {
  if (section.value !== 'chauffeurs' || !chauffeurForm.id_cooperative) { users.value = []; return }
  try { users.value = await cooperativeService.listEligibleChauffeurUsers(chauffeurForm.id_cooperative) }
  catch (errorValue: unknown) { users.value = []; showError(errorValue, 'Impossible de charger les utilisateurs de cette cooperative.') }
}
...
async function loadItem() {
  ...
  if (section.value === 'vehicules') {
    ...
    Object.assign(vehiculeForm, item); existingDocuments.value = item.documents || []
  } else if (section.value === 'chauffeurs') {
    const item: Chauffeur = await chauffeurService.getChauffeur(id.value)
    Object.assign(chauffeurForm, item); await loadEligibleUsers()
    if (item.user && !users.value.some(user => user.id === item.user?.id)) users.value.push({ ...item.user, roles: [], permissions: [], role: 'chauffeur', is_active: true, created_at: '' })
  } else if (section.value === 'marques') Object.assign(marqueForm, await marqueService.getMarque(id.value))
  else Object.assign(modeleForm, await modeleService.getModele(id.value))
}
...
function validateDocumentDrafts() {
  ...
  for (const draft of configured) {
    if (!draft.file) { error.value = 'Chaque document renseigne doit avoir un fichier.'; return false }
    if (draft.date_delivrance && draft.date_expiration && draft.date_expiration < draft.date_delivrance) { error.value = "La date d expiration doit etre posterieure ou egale a la date de delivrance."; return false }
  }
  return true
}
...
async function uploadDrafts(vehicule: Vehicule) {
  for (const draft of documentDrafts.value.filter(item => item.file)) {
    ...
    data.append('is_valid', String(draft.is_valid)); data.append('file', draft.file as File)
...
async function submitMarque() {
  const payload = { nom: marqueForm.nom, description: marqueForm.description || undefined, ...(editing.value ? { is_active: marqueForm.is_active } : {}) }
  if (id.value) await marqueService.updateMarque(id.value, payload); else await marqueService.createMarque(payload)
}
async function submitModele() {
  const payload = { id_marque: modeleForm.id_marque, nom: modeleForm.nom, description: modeleForm.description || undefined, ...(editing.value ? { is_active: modeleForm.is_active } : {}) }
  if (id.value) await modeleService.updateModele(id.value, payload); else await modeleService.createModele(payload)
}
function validateReferenceSelections() {
  if (section.value === 'vehicules' && (!vehiculeForm.id_modele || !vehiculeForm.id_cooperative)) throw new Error('Selectionnez un modele et une cooperative avant de continuer.')
  if (section.value === 'chauffeurs' && (!chauffeurForm.id_user || !chauffeurForm.id_cooperative)) throw new Error('Selectionnez un utilisateur et une cooperative avant de continuer.')
  if (section.value === 'modeles' && !modeleForm.id_marque) throw new Error('Selectionnez une marque avant de continuer.')
}
async function submit() {
  ...
  try {
    ...
    else if (section.value === 'chauffeurs') await submitChauffeur()
...
onMounted(async () => {
  try { await loadReferences(); await loadItem(); if (section.value === 'vehicules') addDocumentDraft() }
  catch (errorValue: unknown) { showError(errorValue, 'Impossible de charger les donnees du formulaire.') }
  finally { loading.value = false }
})
watch(() => chauffeurForm.id_cooperative, () => { void loadEligibleUsers() })
</script>

<template>
  <AppLayout>
    <template #title>{{ title }}</template>
    <div class="page-intro"><div><p class="eyebrow">GESTION DE LA FLOTTE</p><h2>{{ title }}</h2><p>Renseignez les informations puis enregistrez.</p></div><RouterLink class="secondary-button" :to="listPath">Retour a la liste</RouterLink></div>
    <BaseCard>
      <p v-if="loading" class="status-msg">Chargement des donnees en cours...</p>
      <p v-else-if="error && !documentDrafts.length" class="error-banner" role="alert">{{ error }}</p>
      <form v-else class="management-form form-page" @submit.prevent="submit">
        <template v-if="section === 'vehicules'">
          ...
          <div class="field-group"><label for="vehicle-model">Modele <span>*</span></label><select id="vehicle-model" v-model.number="vehiculeForm.id_modele" required><option :value="0" disabled>Choisir un modele</option><option v-for="item in modeles" :key="item.id" :value="item.id">{{ item.nom }}</option></select></div>
          <div class="field-group"><label for="vehicle-brand">Marque</label><input id="vehicle-brand" :value="selectedVehicleBrand?.nom || 'Selectionnez d abord un modele'" class="readonly-field" readonly /></div>
          <div class="field-group"><label for="vehicle-cooperative">Cooperative <span>*</span></label><select id="vehicle-cooperative" v-model.number="vehiculeForm.id_cooperative" required><option :value="0" disabled>Choisir une cooperative</option><option v-for="item in cooperatives" :key="item.id" :value="item.id">{{ item.nom }}</option></select></div>
          ...
          <div class="field-group"><label for="vehicle-state">Etat</label><select id="vehicle-state" v-model="vehiculeForm.etat"><option value="BON_ETAT">Bon etat</option><option value="MOYEN">Moyen</option><option value="A_REPARER">A reparer</option><option value="HORS_SERVICE">Hors service</option></select></div>
          ...
          <div class="field-group field-wide"><label for="vehicle-description">Description</label><textarea id="vehicle-description" v-model="vehiculeForm.description" placeholder="Informations complementaires"></textarea></div>
          <div class="detail-subsection field-wide vehicle-documents-form"><div class="card-heading"><div><h3>Documents du vehicule</h3><p>Ajoutez un ou plusieurs fichiers lors de l enregistrement.</p></div><button class="secondary-button compact-button" type="button" @click="addDocumentDraft">+ Ajouter un document</button></div>
            <div v-for="draft in documentDrafts" :key="draft.key" class="document-draft"><div class="field-group"><label>Type</label><select v-model="draft.type_document"><option value="CARTE_GRISE">Carte grise</option><option value="ASSURANCE">Assurance</option><option value="VISITE_TECHNIQUE">Visite technique</option><option value="AUTRE_DOCUMENT">Autre document</option></select></div><div class="field-group"><label>Numero</label><input v-model="draft.numero_document" placeholder="CG-001" /></div><div class="field-group"><label>Delivrance</label><input v-model="draft.date_delivrance" type="date" /></div><div class="field-group"><label>Expiration</label><input v-model="draft.date_expiration" type="date" /></div><div class="field-group field-wide"><label>Fichier <span>*</span></label><input type="file" accept=".pdf,.png,.jpg,.jpeg,.gif,.webp,.doc,.docx,.xls,.xlsx" @change="onDraftFileChange(draft, $event)" /><span class="field-hint">PDF, image, Word ou Excel - 10 Mo maximum.</span></div><label class="check-field"><input v-model="draft.is_valid" type="checkbox" /> Document valide</label><button v-if="documentDrafts.length > 1" class="table-action danger-action" type="button" @click="removeDocumentDraft(draft.key)">Retirer</button></div>
          </div>
          <div v-if="editing && existingDocuments.length" class="detail-subsection field-wide"><h3>Documents deja rattaches</h3><div class="detail-list"><div v-for="document in existingDocuments" :key="document.id" class="detail-list-row"><strong>{{ documentTypeLabel(document.type_document) }}</strong><span>N° {{ document.numero_document || '-' }} - Expire le {{ formatDocumentDate(document.date_expiration) }}</span><button v-if="document.fichier_path" class="table-action table-link" type="button" @click="openExistingDocument(document)">Ouvrir</button></div></div><span class="field-hint">Pour modifier ou supprimer un document existant, utilisez la fiche detail du vehicule.</span></div>
        </template>
        <template v-else-if="section === 'chauffeurs'">
          <div class="field-group">
            <label for="driver-cooperative">Cooperative <span>*</span></label>
            <select id="driver-cooperative" v-model.number="chauffeurForm.id_cooperative" required>
              <option :value="0" disabled>Choisir une cooperative</option>
              <option v-for="item in cooperatives" :key="item.id" :value="item.id">{{ item.nom }}</option>
            </select>
          </div>
          <div class="field-group">
            <label for="driver-user">Utilisateur <span>*</span></label>
            <select id="driver-user" v-model.number="chauffeurForm.id_user" :disabled="editing || !chauffeurForm.id_cooperative || submitting" required>
              <option :value="0" disabled>{{ chauffeurForm.id_cooperative ? (users.length ? 'Choisir un utilisateur' : 'Aucun utilisateur eligible') : 'Choisir d abord une cooperative' }}</option>
              <option v-for="item in users" :key="item.id" :value="item.id">{{ item.first_name }} {{ item.name }} - #{{ item.id }}</option>
            </select>
          </div>
          <div class="field-group"><label for="driver-license">Numero de permis <span>*</span></label><input id="driver-license" v-model="chauffeurForm.numero_permis" required /></div>
          <div class="field-group"><label for="driver-category">Categorie <span>*</span></label><input id="driver-category" v-model="chauffeurForm.categorie_permis" required placeholder="Ex. D" /></div>
          <div class="field-group"><label for="driver-expiration">Expiration du permis <span>*</span></label><input id="driver-expiration" v-model="chauffeurForm.date_expiration_permis" type="date" required /></div>
          <label class="check-field"><input v-model="chauffeurForm.disponibilite" type="checkbox" /> Disponible</label>
          <label v-if="editing" class="check-field"><input v-model="chauffeurForm.is_active" type="checkbox" /> Actif</label>
        </template>
        <template v-else-if="section === 'marques'"><div class="field-group"><label for="brand-name">Nom <span>*</span></label><input id="brand-name" v-model="marqueForm.nom" required placeholder="Ex. Toyota" /></div><div class="field-group field-wide"><label for="brand-description">Description</label><textarea id="brand-description" v-model="marqueForm.description" placeholder="Informations complementaires"></textarea></div><label v-if="editing" class="check-field"><input v-model="marqueForm.is_active" type="checkbox" /> Active</label></template>
        <template v-else><div class="field-group"><label for="model-brand">Marque <span>*</span></label><select id="model-brand" v-model.number="modeleForm.id_marque" required><option :value="0" disabled>Choisir une marque</option><option v-for="item in marques" :key="item.id" :value="item.id">{{ item.nom }}</option></select></div><div class="field-group"><label for="model-name">Nom du modele <span>*</span></label><input id="model-name" v-model="modeleForm.nom" required placeholder="Ex. Hiace" /></div><div class="field-group field-wide"><label for="model-description">Description</label><textarea id="model-description" v-model="modeleForm.description" placeholder="Informations complementaires"></textarea></div><label v-if="editing" class="check-field"><input v-model="modeleForm.is_active" type="checkbox" /> Actif</label></template>
        <div v-if="error" class="error-banner" role="alert">{{ error }}</div><div class="form-actions"><button class="primary-button compact-button" type="submit" :disabled="submitting">{{ submitting ? 'Enregistrement...' : 'Enregistrer' }}</button><RouterLink class="secondary-button" :to="listPath">Annuler</RouterLink></div>
      </form>
    </BaseCard>
  </AppLayout>
</template>
