...
const description = computed(() => section.value === 'gares'
...
function showError(errorValue: unknown, fallback: string) {
  error.value = userError(errorValue, fallback, 'MANAGEMENT_LIST_ERROR')
  success.value = ''
}
...
async function load() {
  ...
  try {
    if (section.value === 'gares') {
      const result = await managementService.listGares({
        page: page.value,
        page_size: 20,
      ...
      gares.value = result.items
      total.value = result.total
      pages.value = result.pages || 1
    } else {
      const result = await managementService.listCooperatives({
        page: page.value,
        page_size: 20,
        search: search.value || undefined,
        sort_by: 'nom',
      ...
      cooperatives.value = result.items
      total.value = result.total
      pages.value = result.pages || 1
    }
  } catch (errorValue: unknown) {
    showError(errorValue, `Impossible de charger les ${title.value.toLowerCase()}.`)
  } finally {
    loading.value = false
  }
}
...
function changePage(next: number) {
  if (next >= 1 && next <= pages.value && !busyAction.value) {
    page.value = next
    void load()
  }
}
...
async function toggle(item: Gare | Cooperative) {
  ...
  error.value = ''
  try {
    const updated = section.value === 'gares'
      ? await managementService.toggleGare(item.id)
      : await managementService.toggleCooperative(item.id)
...
async function remove(item: Gare | Cooperative) {
  ...
  try {
    if (section.value === 'gares') {
      await managementService.deleteGare(item.id)
      gares.value = gares.value.filter(value => value.id !== item.id)
    } else {
      await managementService.deleteCooperative(item.id)
      cooperatives.value = cooperatives.value.filter(value => value.id !== item.id)
    }
    total.value = Math.max(0, total.value - 1)
    success.value = 'Élément supprimé avec succès.'
  } catch (errorValue: unknown) {
    showError(errorValue, 'Suppression impossible.')
  } finally {
    busyAction.value = null
  }
}
...
<template>
  <AppLayout>
    ...
    <div class="section-links">
      <RouterLink to="/gares" :class="{ active: section === 'gares' }">Gares</RouterLink>
      <RouterLink to="/cooperatives" :class="{ active: section === 'cooperatives' }">Coopératives</RouterLink>
    </div>

    <BaseCard>
      <div class="card-heading">
        <div>
          <h2>{{ title }} enregistrées ({{ total }})</h2>
          <p>Utilisez la recherche, le tri et la pagination pour retrouver rapidement une donnée.</p>
        </div>
      </div>
      ...
      <ListToolbar
        ...
        :sort-label="sortOrder === 'asc' ? 'Tri croissant ↑' : 'Tri décroissant ↓'"
        @search="page = 1; load()"
        @sort="toggleSort"
      />
      <p v-if="loading" class="status-msg" role="status">Chargement des données…</p>
      ...
      <div v-if="!loading && !error" class="table-scroll">
        <table class="data-table">
          <caption>Liste des {{ title.toLowerCase() }}</caption>
          <thead>
            <tr v-if="section === 'gares'"><th>Nom</th><th>Ville</th><th>Adresse</th><th>Statut</th><th>Actions</th></tr>
            <tr v-else><th>Nom</th><th>Sigle</th><th>Ville</th><th>Contact</th><th>Statut</th><th>Actions</th></tr>
          </thead>
          <tbody>
