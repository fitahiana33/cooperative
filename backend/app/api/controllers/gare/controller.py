...
@router.post("/{gare_id}/quais", status_code=status.HTTP_201_CREATED)
def add_quai(
    gare_id: int,
    data: QuaiCreate,
    _: User = Depends(require_permission("GARE_UPDATE")),
...
@router.post("/{gare_id}/zones", status_code=status.HTTP_201_CREATED)
def add_zone(
    gare_id: int,
    data: ZoneCreate,
    _: User = Depends(require_permission("GARE_UPDATE")),
...
def toggle_quai(gare_id: int, quai_id: int, _: User = Depends(require_permission("GARE_UPDATE")), db: Session = Depends(get_db)):
    item = GareService(db).update_quai(gare_id, quai_id)
    return GareService(db).update_quai(gare_id, quai_id, is_active=not item.is_active)
...
def toggle_zone(gare_id: int, zone_id: int, _: User = Depends(require_permission("GARE_UPDATE")), db: Session = Depends(get_db)):
    item = GareService(db).update_zone(gare_id, zone_id)
    return GareService(db).update_zone(gare_id, zone_id, is_active=not item.is_active)
