...
class Vehicule(Base):
    ...
    documents = relationship("VehiculeDocument", back_populates="vehicule", cascade="all, delete-orphan")
    chauffeurs_assignments = relationship("VehiculeChauffeur", back_populates="vehicule", cascade="all, delete-orphan")


class VehiculeDocument(Base):
    __tablename__ = "vehicule_documents"
    __table_args__ = (
        CheckConstraint("type_document IN ('CARTE_GRISE', 'ASSURANCE', 'VISITE_TECHNIQUE')", name="ck_vehicule_document_type"),
    ...
    vehicule = relationship("Vehicule", back_populates="documents")
...
class VehiculeChauffeur(Base):
    ...
    id_vehicule: Mapped[int] = mapped_column(ForeignKey("vehicules.id_vehicule", ondelete="CASCADE"), primary_key=True)
    id_chauffeur: Mapped[int] = mapped_column(ForeignKey("chauffeurs.id_chauffeur", ondelete="CASCADE"), primary_key=True)
    ...
    vehicule = relationship("Vehicule", back_populates="chauffeurs_assignments")
    chauffeur = relationship("Chauffeur", back_populates="vehicules_assignments")
